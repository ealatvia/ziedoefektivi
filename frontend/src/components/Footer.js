...
export default function Footer({ global }) {
  ...
  return (
    <footer className="bg-teal-800" aria-labelledby="footer-heading">
        ...
        <div className="xl:grid xl:grid-cols-3 xl:gap-8">
          ...
          <div className="grid grid-cols-1 gap-x-4 gap-y-8 py-16 xs:grid-cols-2 xl:pt-0 lg:grid-cols-4 xl:col-span-2 xl:mt-0">
            {footer.columns.map((column) => (
              <div key={column.id}>
                <h3 className="px-3 pb-2.5 text-sm font-semibold tracking-wider leading-5 text-teal-100 uppercase">
                  {column.title}
                </h3>
                <ul
                  role="list"
                  className="flex flex-col items-start text-sm text-teal-100"
                >
                  {column.links.map((link) => (
                    <li key={link.id}>
                      <Button
                        ...
                        type="text"
                        size="md"
                        className="!justify-start !font-normal leading-6 !gap-0 text-teal-100"
                        {...link}
                      />
                    </li>
                  ))}
